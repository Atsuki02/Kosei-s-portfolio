...
function Work({ images }: { images: string[] }) {
  ...
  const formatIndex = (value: number) => `Frame ${String(value + 1).padStart(2, "0")}`;
            ...
            {firstColumnImages.map((src: string, index: number) => {
              ...
              >
                ...
                <div className='absolute inset-x-5 top-5 flex items-center justify-between text-[10px] uppercase tracking-[0.32em] text-white mix-blend-difference'>
                  <span className='rounded-full border border-white/20 bg-black/20 px-3 py-1 backdrop-blur-sm'>{formatIndex(index)}</span>
                ...
                />
                <figcaption className='absolute inset-x-5 bottom-5 flex items-end justify-between text-white'>
                  <div className='space-y-1'>
                    <p className='text-xs uppercase tracking-[0.26em] text-white/60'>Editorial</p>
                    <p className='text-lg font-light leading-tight tracking-wide'>Silent Contrast</p>
                  </div>
                  <span className='hidden text-xs uppercase tracking-[0.28em] text-white/60 md:inline'>View</span>
                </figcaption>
              </motion.figure>
            );
            })}
